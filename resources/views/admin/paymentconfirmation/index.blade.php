...
@section('content')
	<div class="box box-warning">
        <div class="box-header with-border">
        	<h3 class="box-title"><i class="fa fa-circle-o"></i> Payment Confirmation</h3>
        </div>
	...
	<script>
		jQuery(document).ready(function($){
	        ...
	        var table = $('#payment_table').DataTable({
	            ...
	            "lengthChange": true,
	            "ajax": {
	                "url": "{{ route('payment_confirmation.index') }}",
	                "type": "POST",
	                "data" : {}
	            },
	            "language": {
	            ...
	            "columns": [
	                ...
	                },
	                {
	                    render : function(data, type, row){
                            if( row.status_payment == '{{ App\Models\Transaction::STATUS_PAYMENT_REJECTED }}' )
                                ...
                                return '<span class="label label-warning">'+ 'Waiting Verification' +'</span>';
                            else if( row.status_payment == '{{ App\Models\Transaction::STATUS_PAYMENT_VERIFIED }}' )
                                return '<span class="label label-verified">'+ row.status_payment +'</span>';
                            else
	                ...
	                {
	                    render : function(data, type, row){
	                        return	'<a href="{{ route('payment_confirmation.index') }}/'+ row.id +'" class="view-btn btn btn-xs btn-primary"><i class="fa fa-eye"> view</i></a>';
	                    },
	                    "width": "10%",
	                    "orderable": false,
	                }
